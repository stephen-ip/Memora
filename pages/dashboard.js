...
export async function getServerSideProps(context) {
  ...
  if (data.user) {
    const matchhistory = await fetch(
      `https://memora-pi.vercel.app/api/stats/matchhistory/`
    ).then(async (response) => {
      let responsejson = await response.json();
      return responsejson;
    });
    const matchhistoryUser = await fetch(
      `https://memora-pi.vercel.app/api/stats/matchhistory/${data.user.username}`,
      {
        method: "GET",
        headers: {
    ...
    if (data.user.placementtest == true) {
      mmseData = await fetch(
        `https://memora-pi.vercel.app/api/stats/alzheimer/mmse/`,
        {
          method: "POST",
          body: JSON.stringify({
