...
//   const [gameStarted, setGameStarted] = useState(false);
  ...
  const [COLORS, setCOLORS] = useState(["#301466", "#301466", "#5C3D99", "#5C3D99", "#38A3A5", "#38A3A5", "#57CC99", "#57CC99", "#80ED99", "#80ED99", "#FFFF00", "#FFFF00"].sort(() => .5 - Math.random()));
  ...
  const handleClickTile = (e) => {
      console.log(flip, e.target.parentNode.childNodes.item(1).style.backgroundColor);
      ...
      } else if (flip && e.target.parentNode.childNodes.item(1).style.backgroundColor != flip.style.backgroundColor) {
          setMoves(moves+1);
  ...
  return (
    ...
    <div className="cardflip__instructions">
        <h2>Instructions</h2>
        <p>
            This is a variation of the classic card flip 
            memory game designed to test episodic memory
            where only one card is revealed at
            once and each second flip that does not match
            will increase your move count by 1.  The goal is
            to match all tiles with the fewest move count.
        </p>
    </div>
      <div className="cardflip__board">
          {COLORS.map((color, i) => {
              return <div key={i} onClick={handleClickTile} className="cardflip__board-tile">
                <div className="cardflip__board-tile-content">
                    <div className="cardflip__board-tile-content-front"></div>
                    <div style={{backgroundColor: color}} className="cardflip__board-tile-content-back"></div>
                </div>
              </div>
          })}
      </div>
