...
export async function getServerSideProps(context) {
  ...
  const data = await fetch(`https://memora-stephen-ip.vercel.app/api/user/${username}`).then(
    async (response) => {
  ...
  if (data.user) {
    for (var i = 0; i < games.length; i++) {
      const game = games[i];
      await fetch(`https://memora-stephen-ip.vercel.app/api/stats/scores/desc/${game}`, {
        method: "GET",
        headers: {
